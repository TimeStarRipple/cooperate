package com.labcontroller.utils;
...
public class LabActivityStateTimeTask{
	...
	public void labActivityEnd(){
		 ...
		 * 获取预约成功的记录
    	 */		
		Iterator<OrderLab> iterator = adminLabService.getOrderLabBySort(2).iterator();
		while(iterator.hasNext()){
			OrderLab labActivity = iterator.next();
			try 
			{
				Date end = dateFormat.parse(labActivity.getOver());
				Date date = new Date();
				
			    if(end.getTime() <= date.getTime())
			    {
			    	labActivity.setSort(3);
			    	adminLabService.overLabById(labActivity.getId());
			    }
